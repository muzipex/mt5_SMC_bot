...
TELEGRAM_TOKEN = "7377915973:AAFjd5zloE8neWr2tzhkirk3uE4GvlArWSE"
...
RISK_PERCENTAGE = 10       # Risk per trade (1% of balance)
STOP_LOSS_PIPS = 50       # Stop loss in pips
TARGET_PROFIT_DOLLARS = 1  # Target profit in dollars
SESSION_FILTER = [(5, 17)] # Trade only during London & New York session (UTC)
...
# Initialize Trading Bot on Request Basis
def initialize_bot(data):
    return MT5SMCBot(data.get("account_number"), data.get("password"), data.get("server"))

# API: Connect to MT5
@app.route('/connect', methods=['POST'])
def connect_mt5():
    data = request.json
    bot = initialize_bot(data)
    success = bot.connect()

    return jsonify({
        "status": "success" if success else "error",
        "message": "Connected to MT5" if success else "Failed to connect"
    })

# API: Retrieve Account Balance
@app.route('/account_balance', methods=['POST'])
def account_balance():
    data = request.json
    bot = initialize_bot(data)
    response = bot.get_account_balance()
...
def market_data():
    ...
    df = bot.get_market_data(data["symbol"], data["timeframe"])
    if df is None:
...
def trade():
    ...
    if not trade_signal:
        return jsonify({"status": "error", "message": "No trade signal detected"})

    bot.place_trade(data["symbol"], trade_signal, data["lot_size"])
    return jsonify({"status": "success", "message": f"Trade executed: {trade_signal} {data['symbol']}"})
